...
This repo is sanitized: site-specific values appear as `<PLACEHOLDER>` tokens
(e.g. `<PBX_FQDN>`, `<TRUNK_SRC_IP>`, `<PBX_ULA>`). Copy
[`site.env.example`](site.env.example) to `site.env` (gitignored), fill in your
real values, and substitute the tokens when you run the commands below.
